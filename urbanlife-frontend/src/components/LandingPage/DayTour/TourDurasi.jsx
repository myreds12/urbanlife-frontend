...
const TourDurasi = ({ durasi }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-md">
      <h2 className="text-xl font-semibold text-gray-800 mb-4">{t("rentcar.duration_price")}</h2>
      <div className="overflow-x-auto">
        <table className="min-w-full table-auto border border-gray-200 rounded-md overflow-hidden">
          <thead className="bg-cyan-600 text-white text-sm uppercase">
            <tr>
              <th className="px-6 py-3 text-left">{t("rentcar.duration")}</th>
              <th className="px-6 py-3 text-left">{t("rentcar.price")}</th>
            </tr>
          </thead>
            ...
            {durasi.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50 transition-all">
                <td className="px-6 py-4 font-medium text-gray-700">
                  {item.durasi.replace('hours', t('rentcar.hours'))}
                </td>
