...
const Navbar = () => {
  ...
  const destinationData = {
    'Top Attractions': topAttractions.length > 0 ? topAttractions : [
    ...
    const fetchTopAttractions = async () => {
      ...
      try {
        ...
        const formattedData = rawData.map(item => ({
          id: item.id,
          name: item.nama,
          location: `${item.lokasi?.nama || 'Unknown'}, ${item.lokasi?.negara?.nama || 'Unknown'}`,
          image: item.file_url
            ? `${apiClient.defaults.baseURL.replace(/\/$/, "")}/public/${item.file_url.replace(/\\/g, "/").replace(/^uploads\//, "")}`
        ...
        console.error("❌ API Error - using dummy data fallback:", error.response?.status, error.message); // New comment: Detailed error log
        setTopAttractions([]);
      } finally {
        setIsLoading(false);
      }
  ...
  const handleDestinationClick = (destination) => {
    ...
    console.log("Navigating to DaytourDetail with data:", detailData); // New comment: Debug navigation
    navigate(`/DaytourDetail?id=${destination.id}`, { state: detailData });
  };
